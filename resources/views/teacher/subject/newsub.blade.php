...
<main class="flex-grow">
    <section class="container mx-auto px-4 py-16 max-w-xl">
      <h2 class="text-2xl font-bold mb-8 text-center">Create New Subject</h2>
      <form class="space-y-6" action="{{route('subject.storesub')}}" method="post">
        @csrf
        ...
        <div>
          ...
          <input type="text" id="subject_code" name="subject_code" class="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div>
          ...
          <input type="number" id="credit" name="credit" min="1" class="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
          value = "{{ old('credit', ' ')}}">
          @error('credit')
          <div class="text-red-500">{{ $message }}</div>
          @enderror
